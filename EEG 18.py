...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.output = nn.Linear(32 * 210, 21)  # 这里的32*210是数字，不代表二维32维度乘以210，训练数据有变化的时候，这里必须要改
    ...
    def forward(self, x):
        ...
        # out.view平铺到一维，类似于keras中的flatten
        out1 = out1.view(out1.size(0), -1)  # pytorch的展平操作是在forward中定义的，在这里维度变为二维20*（32*210）
...
eegdata_1 = np.random.uniform(0, 1, size = (10, 16, int((tbase + ttrial) * fs*10)))
eegdata_2 = np.random.uniform(0, 1, size = (10, 16, int((tbase + ttrial) * fs*10)))
eegdata_1_label = np.zeros(10)
eegdata_2_label = np.ones(10)
eegdata = np.vstack((eegdata_1,eegdata_2))
eegdata_label = np.hstack((eegdata_1_label,eegdata_2_label))
#print(eegdata_label)
#eegdata = np.random.rand(20, 16, int((tbase + ttrial) * fs))
#eegdata = eegdata2.astype(np.int)
#print('# test eegdata:', eegdata[0, 0][:2], '\n')
#print(eegdata[0].shape)
tbase = 0.2  # 基线长度
ttrial = 0.5  # 关注的脑电时间段
fs = 1200  # 采样率
...
for temp in epochdata:
    ...
    #print(round((eegdata[0, 0][i] - tbase) * fs) + int((tbase + ttrial) * fs))
    a = round((eegdata[0, 0][i] - tbase) * fs)
    a = abs(a)
    #print('a',round(a))
    b = a + int((tbase + ttrial) * fs)
    epochdata[i, :, :] = eegdata[0][:, a:b]
    i += 1
...
#print('# test epoched data 1111:',epochdata[0,0,1],epochdata[1,0,1])
...
labels = np.zeros(20)
labels = torch.LongTensor(eegdata_label[:])
...
'''
for temp in epochdata:
    epochdata[i, :, :] = eegdata[0, 0][:, round((eegdata[0, 0][i, 1] - tbase) * fs):round(
        (eegdata[0, 0][i, 1] - tbase) * fs) + int((tbase + ttrial) * fs)]
    i += 1
'''
for temp in epochdata:
...
#loss_func2 = nn.CrossEntropyLoss()  # 损失函数选择交叉熵损失函数
loss_func1 = nn.L1Loss()
for epoch in range(30):
    for step, (b_x, b_y) in enumerate(train_loader):
        out, out1 = cnn(b_x)
        # b_y必须为long型
        #print(out[0,0],out1[0,0])
        loss = loss_func(out, b_y)
        loss2 = loss_func1(out1[:,:20], b_y)
        loss = (loss + loss2) / 2
        ...
        if step % 20 == 0:
            ...
            pred_y = torch.max(test_output[0], 1)[1].data.numpy()
...
test_output, test_output1 = cnn(test_x[:20])
...
pred_y1 = torch.max(test_output1[0], 1)[1].data.numpy()
